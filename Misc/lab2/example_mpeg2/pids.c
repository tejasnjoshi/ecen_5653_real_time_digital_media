...
int findAllTiming(FILE *fp, unsigned int packetLimit, playbackContext *pbPtr)
{
    ...
    do  // Scan file for PCRs
    {
        ...
        else // Parse PID and adaptation fields
        {
            ...
            {
                printf("\nMPEG packet %llu, AdaptFieldCtl=0x%x, adaptFieldLen=%d, adaptFlag=0x%x, pcrFlag=%d, opcrFlag=%d, discFlag=%d\n",
                       mpegPackets, AdaptFieldCtl, AdaptLength, AdaptFlags, pcrFlag, opcrFlag, discIndicator);
                numPCRs++;
