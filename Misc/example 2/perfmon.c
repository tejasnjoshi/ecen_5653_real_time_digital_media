...
int main( int argc, char *argv[] )
{
   ...
   if(argc == 2)
   {
	//takes the second argument as the number of iterations
      sscanf(argv[1], "%llu", &reqIterations);
     //calculation for the number of iterations
	...
	//if no arguments are provided uses default iteration value
   else if(argc == 1)
   ...
   startTOD=readTOD(); //Grab the timer start count
   usleep(1000000);	//sleep for 1 second
   stopTOD=readTOD();
   //DT stores the value from the elapsedTOD (from perflib.c)
